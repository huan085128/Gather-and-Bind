...
class EvalConfig:
    output_path: Path = Path(
        "/ai/workshop/pipeline/val_output3/objects")
    metrics_save_path: Path = Path(
        "/ai/workshop/pipeline/val_output3/metrics")
...
def run(config: EvalConfig):
    ...
    with open(config.metrics_save_path / "clip_raw_metrics_objects.json", 'w') as f:
        json.dump(results_per_prompt, f, sort_keys=True, indent=4)
    with open(config.metrics_save_path / "clip_aggregated_metrics_objects.json", 'w') as f:
