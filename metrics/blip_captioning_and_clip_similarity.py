...
@dataclass
class EvalConfig:
    output_path: Path = Path("/ai/workshop/pipeline/val_output3/objects")
    metrics_save_path: Path = Path("/ai/workshop/pipeline/val_output3/metrics/")
...
def run(config: EvalConfig):
    ...
    prompts = [p.replace("_", " ") for p in prompts]
    ...
    for prompt in tqdm(prompts):
        ...
        image_paths = [p for p in (config.output_path / prompt.replace(' ', '_')).rglob('*') if p.suffix in ['.png', '.jpg']]
    ...
    with open(config.metrics_save_path / "blip_raw_metrics_objects.json", 'w') as f:
        json.dump(results_per_prompt, f, sort_keys=True, indent=4)
    with open(config.metrics_save_path / "blip_aggregated_animals_objects.json", 'w') as f:
